...
const { SFI } = require('../../../../app/constants/scheme-sources')
...
describe('get AP vendor line', () => {
  test('should return an array of 30 elements if payment request has schedule', () => {
    const line = getVendorLineAP(paymentRequest, batch, source, highestValueLine)
  ...
  test('should return item 14 as empty string if BPS', () => {
    const line = getVendorLineAP(bpsPaymentRequest, batch, source, highestValueLine)
    expect(line[13]).toBe('')
  })

  test('should return item 14 as empty string if FDMR', () => {
    const line = getVendorLineAP(fdmrPaymentRequest, batch, source, highestValueLine)
    expect(line[13]).toBe('')
  })
...
describe('get AR vendor line', () => {
  ...
  test('should return array of 22 items', () => {
    const line = getVendorLineAR(paymentRequest, batch, source, lowestValueLine)
